...
void
assert_eq(Lhs&& lhs, Rhs&& rhs, std::string_view head, std::format_string<Args...> fmt, Args&&... args)
{
  if (lhs != rhs) [[unlikely]] {
...
template<typename Lhs, typename Rhs>
  requires(not formattable_concept<Lhs, Rhs>)
void
assert_eq(Lhs&& lhs, Rhs&& rhs, std::string_view head)
{
  if (lhs != rhs) [[unlikely]] {
...
// clang-format off
#define ASSERT_EQ(LHS, RHS, ...) kaspan::assert_eq (LHS, RHS, \
"[ASSERTION FAILED]\n"                                \
"  Condition  : " #LHS " == " #RHS "\n"               \
"  File       : " __FILE__ "\n"                       \
