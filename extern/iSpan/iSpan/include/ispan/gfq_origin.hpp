...
gfq_origin(
  ...
  index_t*       sub_vertices,

  index_t const*  fw_beg_pos,
  vertex_t const* fw_csr,
  index_t const*  bw_beg_pos,
  ...
  vertex_t* sub_vertices_inverse)
{
  ...
  for (vertex_t sub_u = 0; sub_u < sub_n; ++sub_u) {
    ...
    // fw
    {
      auto const beg = fw_beg_pos[u];
      auto const end = fw_beg_pos[u + 1];
    ...
    // bw
    {
      auto const beg = bw_beg_pos[u];
      auto const end = bw_beg_pos[u + 1];
      for (auto it = beg; it < end; ++it) {
  ...
  work_comm[world_rank]  = sub_fw_m;

  std::cout << "sub v_count, " << front_comm[world_rank] << ", sub e_count, " << work_comm[world_rank] << "," << sub_bw_m << "\n";
}
