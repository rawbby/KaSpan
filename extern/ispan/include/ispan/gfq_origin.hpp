...
gfq_origin(index_t const   n,
           ...
           vertex_t* sub_fw_head,
           vertex_t* sub_fw_csr,
           vertex_t* sub_bw_head,
           vertex_t* sub_bw_csr)
{
  sub_n = 0;
  ...
  for (vertex_t sub_u = 0; sub_u < sub_n; ++sub_u) {
    ...
    for (auto it = beg; it < end; ++it) {
      ...
      if (scc_id[v] == scc_id_undecided) sub_fw_csr[sub_fw_m++] = sub_vertices_inverse[v];
    }
  ...
  for (vertex_t sub_u = 0; sub_u < sub_n; ++sub_u) {
    ...
    for (auto it = beg; it < end; ++it) {
      ...
      if (scc_id[v] == scc_id_undecided) sub_bw_csr[sub_bw_m++] = sub_vertices_inverse[v];
    }
