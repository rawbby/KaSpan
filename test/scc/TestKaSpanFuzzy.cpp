...
main(int argc, char** argv)
{
  ...
  for (size_t n = 32; n < 64; n += 8) {
    ...
    ASSERT_TRY(auto scc_id_ispan, U64Buffer::zeroes(graph.partition.size()));
    ...
    for (size_t k = 0; k < graph.partition.size(); ++k)
      ss << ' ' << std::right << std::setw(2) << graph.partition.select(k);
    ss << "\n  scc_id_orig   :";
    for (size_t k = 0; k < graph.partition.size(); ++k)
