...
public:
  ...
  static constexpr auto failure() -> result { return result(ErrorCode::ERROR); }

  static constexpr auto failure(ErrorCode error) -> result { return result(error); }

  [[nodiscard]] constexpr auto has_value() const -> bool { return std::holds_alternative<V>(data); }

  constexpr explicit operator bool() const { return has_value(); }

  constexpr auto error() -> ErrorCode { return std::get<ErrorCode>(data); }

  [[nodiscard]] constexpr auto error() const -> ErrorCode { return std::get<ErrorCode>(data); }
