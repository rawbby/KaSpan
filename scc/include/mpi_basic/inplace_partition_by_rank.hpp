 ...
 * @tparam Fn Callable with signature i32(T const&), returning the destination rank.
 */
template<typename T, typename Fn>
  requires(std::convertible_to<std::invoke_result_t<Fn, T const&>, i32>)
void
inplace_partition_by_rank(T* send_buffer, MPI_Count const* send_counts, MPI_Aint* send_displs, Fn&& rank_of)
{
  ...
  send_displs[0] = 0;
  for (i32 r = 1; r < world_size; ++r) { send_displs[r] = send_displs[r - 1] + static_cast<MPI_Aint>(send_counts[r - 1]); }
