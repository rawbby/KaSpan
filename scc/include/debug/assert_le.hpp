...
void
assert_le(Lhs&& lhs, Rhs&& rhs, std::string_view head, std::format_string<Args...> fmt, Args&&... args)
{
  if (lhs > rhs) [[unlikely]] {
...
template<typename Lhs, typename Rhs>
  requires(not FormattableConcept<Lhs, Rhs>)
void
assert_le(Lhs&& lhs, Rhs&& rhs, std::string_view head)
{
  if (lhs > rhs) [[unlikely]] {
...
// clang-format off
#define ASSERT_LE(LHS, RHS, ...) assert_le (LHS, RHS, \
"[ASSERTION FAILED]\n"                                \
"  Condition  : " #LHS " <= " #RHS "\n"               \
"  File       : " __FILE__ "\n"                       \
