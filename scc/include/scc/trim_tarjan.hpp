...
choose_contains_fn(std::span<vertex_t> span)
{
  ...
  if (span.size_bytes() >= 256) [[unlikely]] {
    ...
    return RootAndFn{ span[0], binary_contains };
  }

  return RootAndFn{ *std::ranges::min_element(span), linear_contains };
}
...
trim_tarjan(Part const& part, index_t const* fw_head, vertex_t const* fw_csr, index_t const* bw_head, vertex_t const* bw_csr, vertex_t* scc_id, Filter filter = no_filter)
{
  ...
  tarjan(part, fw_head, fw_csr, [=, &decided, &filter](vertex_t* cbeg, vertex_t* cend) -> void {
