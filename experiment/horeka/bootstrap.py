...
def get_header(comp, s_name, scaling, rel_path, is_rwd=False):
    ...
    name_base = rel_path.replace('/', '_').replace('.sh', '')
    out_err_name = name_base
    job_name = name_base
    run_script = "run_rwd.sh" if is_rwd else "run_generic.sh"
...
def get_generic_body(comp, s_name, scaling, g_name, kagen):
    ...
    if scaling == 'weak':
        if s_name == 'single':
            body = f"\nfor np in 76 38 16 8 4 2 1; do\nfor local_n in {local_n_list}; do\nfor d in {d_list}; do\nn=$(( np * local_n ))\nm=$(( n * d / 100 ))\nkagen_string=\"{kagen}\"\noutput_file=\"${{scaling}}_${{app_name}}_np${{np}}_{g_name}_localn${{local_n}}_d${{d}}.json\"\n"
            ...
            for np_val, nodes_val in [(152, 2), (304, 4)]:
                ...
                body += f'run_async run_generic "$app" "${{scaling}}_${{app_name}}_np{np_val}_{g_name}_localn${{local_n}}_d${{d}}.json" "$kagen_string" "$app_name" {np_val} "$local_n" "$d" "{g_name}" "{run_args}"{" --threads 76" if is_hpc else ""}\n'
            body += f"wait_all\ndone\ndone\n# np=532\nfor local_n in {local_n_list}; do\nfor d in {d_list}; do\nn=$(( 532 * local_n ))\nm=$(( n * d / 100 ))\nkagen_string=\"{kagen}\"\n"
            ...
            body += f'run_generic "$app" "${{scaling}}_${{app_name}}_np532_{g_name}_localn${{local_n}}_d${{d}}.json" "$kagen_string" "$app_name" 532 "$local_n" "$d" "{g_name}" "{run_args}"{" --threads 76" if is_hpc else ""}\ndone\ndone\n'
        else:
            body = f"\nnp={cfg['ntasks']}\nfor local_n in {local_n_list}; do\nfor d in {d_list}; do\nn=$(( np * local_n ))\n"
        ...
        if s_name == 'single':
            body = f"\nfor np in 76 38 16 8 4 2 1; do\nfor n in {n_list}; do\nfor d in {d_list}; do\nm=$(( n * d / 100 ))\nkagen_string=\"{kagen}\"\noutput_file=\"${{scaling}}_${{app_name}}_np${{np}}_{g_name}_n${{n}}_d${{d}}.json\"\n"
            run_args = "--nodes=1 --ntasks=$np --cpus-per-task=1 --hint=nomultithread" if not is_hpc else "--nodes=1 --ntasks=1 --ntasks-per-socket=1 --cpus-per-task=$np"
            ...
            for np_val, nodes_val in [(152, 2), (304, 4)]:
                run_args = f'--nodes={nodes_val} --ntasks={np_val if not is_hpc else nodes_val} --cpus-per-task={1 if not is_hpc else 76}{" --hint=nomultithread" if not is_hpc else ""}'
                body += f'run_async run_generic "$app" "${{scaling}}_${{app_name}}_np{np_val}_{g_name}_n${{n}}_d${{d}}.json" "$kagen_string" "$app_name" {np_val} "$n" "$d" "{g_name}" "{run_args}"{" --threads 76" if is_hpc else ""}\n'
            body += f"wait_all\ndone\ndone\n# np=532\nfor n in {n_list}; do\nfor d in {d_list}; do\nm=$(( n * d / 100 ))\nkagen_string=\"{kagen}\"\n"
            run_args = f'--nodes=7 --ntasks={532 if not is_hpc else 7} --cpus-per-task={1 if not is_hpc else 76}{" --hint=nomultithread" if not is_hpc else ""}'
            body += f'run_generic "$app" "${{scaling}}_${{app_name}}_np532_{g_name}_n${{n}}_d${{d}}.json" "$kagen_string" "$app_name" 532 "$n" "$d" "{g_name}" "{run_args}"{" --threads 76" if is_hpc else ""}\ndone\ndone\n'
        else:  # 1064, 2052, 4104
            body = f"\nnp={cfg['ntasks']}\nfor n in {n_list}; do\nfor d in {d_list}; do\nm=$(( n * d / 100 ))\nkagen_string=\"{kagen}\"\noutput_file=\"${{scaling}}_${{app_name}}_np${{np}}_{g_name}_n${{n}}_d${{d}}.json\"\n"
...
def get_rwd_body(comp, s_name):
    ...
    if s_name == 'single':
        ...
        for np_val, nodes_val in [(152, 2), (304, 4)]:
            run_args = f'--nodes={nodes_val} --ntasks={np_val if not is_hpc else nodes_val} --cpus-per-task={1 if not is_hpc else 76}{" --hint=nomultithread" if not is_hpc else ""}'
            body += f'run_async run_rwd "$app" "${{scaling}}_${{app_name}}_np{np_val}_${{manifest_name}}.json" "$manifest" "$app_name" {np_val} "$manifest_name" "{run_args}"{" --threads 76" if is_hpc else ""}\n'
        body += "wait_all\ndone\nfor manifest in \"${rwd[@]}\"; do\nmanifest_name=\"$(basename \"${manifest%.manifest}\")\"\n"
        run_args = f'--nodes=7 --ntasks={532 if not is_hpc else 7} --cpus-per-task={1 if not is_hpc else 76}{" --hint=nomultithread" if not is_hpc else ""}'
        body += f'run_rwd "$app" "${{scaling}}_${{app_name}}_np532_${{manifest_name}}.json" "$manifest" "$app_name" 532 "$manifest_name" "{run_args}"{" --threads 76" if is_hpc else ""}\ndone\n'
    else:  # 1064, 2052, 4104
        np_val = cfg['ntasks']
        body += f"manifest_name=\"$(basename \"${{manifest%.manifest}}\")\"\noutput_file=\"${{scaling}}_${{app_name}}_np{np_val}_${{manifest_name}}.json\"\n"
    ...
    for g_name, kagen in GENERICS.items():
        for comp in APPS:
            for s_name in CONFIGS:
                rel_path = f"{scaling}/{g_name}/{comp}_{s_name}.sh"
                header = get_header(comp, s_name, scaling, rel_path)
...
def generate_rwd_experiments():
    for comp in APPS:
        for s_name in CONFIGS:
            rel_path = f"strong/rwd/{comp}_{s_name}.sh"
