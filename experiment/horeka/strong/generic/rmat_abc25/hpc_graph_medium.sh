...
for n in 1000000000 100000000 10000000; do
  for d in 90 100 200 400; do
    ...
    run_async run_generic "$app" "${app_name}_rmat_abc25_np152_n${n}_d${d}.json" "$kagen_string" "$app_name" 152 "$n" "$d" "rmat_abc25" "--nodes=2 --ntasks=2 --cpus-per-task=76" --threads 76
    run_async run_generic "$app" "${app_name}_rmat_abc25_np304_n${n}_d${d}.json" "$kagen_string" "$app_name" 304 "$n" "$d" "rmat_abc25" "--nodes=4 --ntasks=4 --cpus-per-task=76" --threads 76
    wait_all
  done
done
