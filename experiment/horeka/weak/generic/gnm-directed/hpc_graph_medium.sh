...
run_async run_generic "$app" "${app_name}_gnm-directed_np152_n${local_n}_d${d}.json" "$kagen_string" "$app_name" 152 "$local_n" "$d" "gnm-directed" "--nodes=2 --ntasks=2 --cpus-per-task=76" --threads 76
n=$(( 304 * local_n ))
m=$(( n * d / 100 ))
kagen_string="gnm-directed;n=${n};m=${m};seed=13"
run_async run_generic "$app" "${app_name}_gnm-directed_np304_n${local_n}_d${d}.json" "$kagen_string" "$app_name" 304 "$local_n" "$d" "gnm-directed" "--nodes=4 --ntasks=4 --cpus-per-task=76" --threads 76
wait_all
done
done
# np=532
