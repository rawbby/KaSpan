...
output_file="${app_name}_gnm-directed_np1064.json"
if [[ -s "$output_file" ]]; then
  echo "[SKIPPING] ${app_name} NP=1064 Graph=gnm-directed"
else
  echo "[STARTING] ${app_name} NP=1064 Graph=gnm-directed"
  srun                   \
    --time=3:00          \
    --oom-kill-step=1    \
    --mpi=pmi2           \
  ...
  if [[ $ec -ne 0 ]]; then
    [[ $ec -eq 137 ]] && ec="${ec} (oom)"
    echo "[FAILURE] ${app_name} NP=1064 Graph=gnm-directed ec=${ec}"
  else
    echo "[SUCCESS] ${app_name} NP=1064 Graph=gnm-directed"
  fi
fi
