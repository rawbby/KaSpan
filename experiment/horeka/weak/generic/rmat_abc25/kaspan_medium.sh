...
for n in 150000 300000 600000; do
  for d in 90 100 200 400; do
    ...
    output_file="${app_name}_rmat_abc25_np152_n${n}_d${d}.json"
    if [[ -s "$output_file" ]]; then
      echo "[SKIPPING] ${app_name} np=152 n=${n} d=${d} graph=rmat_abc25"
    else
      echo "[STARTING] ${app_name} np=152 n=${n} d=${d} graph=rmat_abc25"
      srun                   \
    --time=3:00          \
    --oom-kill-step=1    \
    --mpi=pmi2           \
...
for n in 150000 300000 600000; do
  for d in 90 100 200 400; do
    ...
    kagen_string="rmat;directed;n=${total_n};m=${total_m};a=0.25;b=0.25;c=0.25;seed=13"
    output_file="${app_name}_rmat_abc25_np304_n${n}_d${d}.json"
    if [[ -s "$output_file" ]]; then
      echo "[SKIPPING] ${app_name} np=304 n=${n} d=${d} graph=rmat_abc25"
    else
      echo "[STARTING] ${app_name} np=304 n=${n} d=${d} graph=rmat_abc25"
      srun                   \
    --time=3:00          \
    --oom-kill-step=1    \
    --mpi=pmi2           \
