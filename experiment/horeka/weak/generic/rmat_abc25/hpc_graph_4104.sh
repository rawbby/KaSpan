...
output_file="${app_name}_rmat_abc25_np4104.json"
if [[ -s "$output_file" ]]; then
  echo "[SKIPPING] ${app_name} NP=4104 Graph=rmat_abc25"
else
  echo "[STARTING] ${app_name} NP=4104 Graph=rmat_abc25"
  srun                   \
    --time=3:00          \
    --oom-kill-step=1    \
    --mpi=pmi2           \
  ...
  if [[ $ec -ne 0 ]]; then
    [[ $ec -eq 137 ]] && ec="${ec} (oom)"
    echo "[FAILURE] ${app_name} NP=4104 Graph=rmat_abc25 ec=${ec}"
  else
    echo "[SUCCESS] ${app_name} NP=4104 Graph=rmat_abc25"
  fi
fi
