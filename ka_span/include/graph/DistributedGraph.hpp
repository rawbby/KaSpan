...
struct DistributedGraph
{
  ...
  DistributedGraph(Partition partition, u64 node_count, u64 edge_count)
    : partition(partition)
  ...
  auto operator==(DistributedGraph const&) const -> bool = default;
  auto operator!=(DistributedGraph const&) const -> bool = default;

  static auto load(kamping::Communicator<> const& comm, Partition const& partition, Manifest const& manifest) -> Result<DistributedGraph>
  {
    auto const n      = manifest.graph_node_count;
    auto const m      = manifest.graph_edge_count;
    auto const endian = manifest.graph_endian;

    DistributedGraph result{ partition, comm.rank(), comm.size() };
    ...
    RESULT_TRY(load_dir(manifest.fw_head_path, manifest.fw_csr_path, result.fw_head, result.fw_csr));
    RESULT_TRY(load_dir(manifest.bw_head_path, manifest.bw_csr_path, result.bw_head, result.bw_csr));
