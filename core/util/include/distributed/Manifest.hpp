...
parse_kv_map(const std::filesystem::path& file)
{
  ...
  while (std::getline(in, line)) {
    ...
    const auto value = value_start < line.size() ? line.substr(value_start) : std::string{};
...
struct Manifest
{
  ...
  std::size_t graph_head_bytes;
  std::size_t graph_csr_bytes;
  ...
  load(const std::filesystem::path& file)
  {
    ...
    RESULT_TRY(const auto graph_head_bytes_str, get_value_str("graph.head.bytes"));
    RESULT_TRY(const auto graph_csr_bytes_str, get_value_str("graph.csr.bytes"));
    ...
    RESULT_TRY(manifest.graph_head_bytes, parse_int<std::size_t>(graph_head_bytes_str));
    RESULT_TRY(manifest.graph_csr_bytes, parse_int<std::size_t>(graph_csr_bytes_str));
