...
struct Graph
{
  ...
  void foreach_fw_edge(vertex_t u, Fn&& fn) const
  {
    auto const beg = fw_head.get(u);
    auto const end = fw_head.get(u + 1);
    for (index_t it = beg; it < end; ++it)
      fn(u, fw_csr[it]);
  }

  template<typename Fn>
  void foreach_bw_edge(vertex_t u, Fn&& fn) const
  {
    auto const beg = bw_head.get(u);
    auto const end = bw_head.get(u + 1);
    for (index_t it = beg; it < end; ++it)
      fn(u, bw_csr[it]);
  }
